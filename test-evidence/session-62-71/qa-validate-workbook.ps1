...
$safety = 'C:\Users\Lenovo\AppData\Local\Temp\UNI-NEXUS-QA-20260914-234817\Pengujian UNI-NEXUS.xlsx'
$mainNs = 'http://schemas.openxmlformats.org/spreadsheetml/2006/main'
$target = @{}; foreach ($row in 62..71) { $target["F$row"] = $true; $target["H$row"] = $true }
...
try {
    ...
    foreach($row in 62..71) { Assert-Equal ($after.Cells["F$row"] -eq 'Bug/Failed') "Status mismatch F$row"; Assert-Equal (-not [string]::IsNullOrWhiteSpace($after.Cells["H$row"])) "Note missing H$row" }
    ...
    Write-Output 'Workbook integrity validation: PASS'; Write-Output ('Final SHA-256: '+$hash); Write-Output 'Non-target ZIP entries changed: 0'; Write-Output 'Target cells changed: F62:F71 and H62:H71'; Write-Output 'Historical/completed rows unchanged: 34,39,40,41,42-61'
