...
export const studioFinanceApi = {
  ...
  uploadExpenseReceipt: (id: number, file: File) => { const form = new FormData(); form.set('file', file); return api.post<{ id: number; receipt_path: string; file_name: string }>(`${BASE}/expenses/${id}/receipt`, form); },
  downloadExpenseReceipt: (id: number) => api.getBlob(`${BASE}/expenses/${id}/receipt`),
  removeExpenseReceipt: (id: number) => api.delete<{ id: number; receipt_path: null }>(`${BASE}/expenses/${id}/receipt`),
