...
import { ChevronDown, Download, Filter, Plus, RotateCcw, X } from 'lucide-react';
...
import { OrdersDateTimeInput, OrdersSearchInput } from './components/OrderInputs';
    ...
    <Card className="flex min-h-0 flex-1 overflow-hidden">
      <div className="flex min-h-0 w-full flex-col">
        <div className="space-y-4 border-b border-[var(--nexus-border)] bg-[var(--nexus-cream-soft)]/40 p-4 sm:p-5">
          <OrdersSearchInput placeholder="Cari ID, pelanggan, produk, atau ID marketplace..." value={filters.search || ''} onChange={value => updateFilter('search', value || undefined)} />
          ...
          {advancedOpen && <div className="grid grid-cols-1 gap-3 border-t border-[var(--nexus-border)] pt-4 sm:grid-cols-2"><label className="orders-field"><span className="orders-field-label">Tanggal mulai</span><OrdersDateTimeInput type="date" value={filters.dateFrom || ''} onChange={value => updateFilter('dateFrom', value || undefined)} /></label><label className="orders-field"><span className="orders-field-label">Tanggal akhir</span><OrdersDateTimeInput type="date" value={filters.dateTo || ''} onChange={value => updateFilter('dateTo', value || undefined)} /></label></div>}
