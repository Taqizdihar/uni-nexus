...
export function ExpensesPage() {
  ...
  const submitPay = async (event: React.FormEvent) => {
    ...
    try { await craftFinanceApi.payExpense(payTarget.id, { treasury_account_id: Number(payForm.treasury_account_id), payment_date: payForm.payment_date }); setNotice(`Pengeluaran ${payTarget.expense_code} dibayar.`); setPayTarget(null); load(); }
  ...
  const submitReverse = async (event: React.FormEvent) => {
    ...
    try { await craftFinanceApi.reverseExpense(reverseTarget.id, reverseForm); setNotice(`Pengeluaran ${reverseTarget.expense_code} dibalik.`); setReverseTarget(null); load(); }
    catch (cause) { setFormError(getError(cause)); } finally { setWorking(false); }
  };
