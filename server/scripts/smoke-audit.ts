...
const assertOk = (condition: unknown, message: string): asserts condition => assert.equal(Boolean(condition), true, message);
...
async function main() {
  ...
  try {
    ...
    const login = async (handle: string) => { const { response, body } = await request('/auth/login', { method: 'POST', body: JSON.stringify({ usernameOrEmail: handle, password }) }); assert.equal(response.status, 200, `Login failed for ${handle}: ${body?.error?.message || ''}`); return String(body.data.token); };
    ...
    let result = await request(`/audit?q=${encodeURIComponent(suffix)}`, { headers: auth(token) }); assert.equal(result.response.status, 200); assertOk(result.body.data.items.some((item: any) => item.entity?.code === `GLOBAL-${suffix}`), 'Global Audit row is not visible.'); assertOk(result.body.data.items.some((item: any) => item.entity?.code === `CRAFT-${suffix}`), 'Craft Audit row is not visible.'); assertOk(!result.body.data.items.some((item: any) => item.entity?.code === `STUDIO-${suffix}`), 'Studio Audit row leaked to Craft-only user.');
    result = await request('/audit', { headers: auth(deniedToken) }); assert.equal(result.response.status, 403, 'User without audit.read was allowed to list Audit.');
    result = await request(`/audit?workspace=craft&module=smoke_craft&action=fixture.update&action_group=update&entity_type=fixture&user_id=${auditorId}&q=${encodeURIComponent(`CRAFT-${suffix}`)}&from=2020-01-01&to=2099-01-01`, { headers: auth(token) }); assert.equal(result.response.status, 200); assert.equal(result.body.data.items.length, 1, 'Scoped Audit filters did not yield the Craft fixture.');
    result = await request('/audit/meta?workspace=all', { headers: auth(token) }); assert.equal(result.response.status, 200); assertOk(!result.body.data.modules.some((item: any) => item.code === 'smoke_studio'), 'Metadata leaked inaccessible Studio rows.');
    result = await request('/audit/summary?workspace=all', { headers: auth(token) }); assert.equal(result.response.status, 200); assertOk(result.body.data.total_in_range >= 4, 'Audit summary did not count accessible rows.');
    result = await request(`/audit/${auditIds[2]}`, { headers: auth(token) }); assert.equal(result.response.status, 404, 'Inaccessible Audit detail did not return 404.');
    result = await request(`/audit/${auditIds[3]}`, { headers: auth(token) }); assert.equal(result.response.status, 200); const secretText = JSON.stringify(result.body.data); assertOk(secretText.includes('[REDACTED]') && !secretText.includes('never-return-this') && !secretText.includes('also-never-return-this'), 'New Audit write redaction failed.');
    result = await request(`/audit/${auditIds[4]}`, { headers: auth(token) }); assert.equal(result.response.status, 200); const legacyText = JSON.stringify(result.body.data); assertOk(legacyText.includes('[REDACTED]') && !legacyText.includes('legacy-secret') && !legacyText.includes('legacy-token'), 'Legacy Audit read redaction failed.');
    result = await request('/audit/export?format=csv', { headers: auth(viewToken) }); assert.equal(result.response.status, 403, 'Export did not require reports.export.');
    result = await request(`/audit/export?format=csv&q=${encodeURIComponent(suffix)}`, { headers: auth(token) }); assert.equal(result.response.status, 200); assertOk(result.text.includes('[REDACTED]') && !result.text.includes('never-return-this'), 'CSV Audit export was not redacted.');
    result = await request(`/audit/export?format=xlsx&q=${encodeURIComponent(suffix)}`, { headers: auth(token) }); assert.equal(result.response.status, 200); assertOk(result.response.headers.get('content-type')?.includes('spreadsheetml'), 'XLSX Audit export failed.');
    result = await request('/audit/1', { method: 'DELETE', headers: auth(token) }); assert.equal(result.response.status, 404, 'Audit API exposed a mutation route.');
    const sessionKey = randomUUID(); result = await request('/presence/heartbeat', { method: 'POST', headers: auth(token), body: JSON.stringify({ session_key: sessionKey, workspace_code: 'craft' }) }); assert.equal(result.response.status, 200); result = await request('/auth/logout', { method: 'POST', headers: auth(token), body: JSON.stringify({ session_key: sessionKey }) }); assert.equal(result.response.status, 200);
    ...
    await UsersService.updateStatus(deniedId, 'suspended', auditorId); const [statusAudit]: any = await pool.execute("SELECT entity_type,entity_id,old_values,new_values FROM audit_logs WHERE user_id=? AND action_code='status_change' ORDER BY id DESC LIMIT 1", [auditorId]); assert.equal(Number(statusAudit[0].entity_id), deniedId); assert.equal(statusAudit[0].entity_type, 'user'); assertOk(JSON.stringify(statusAudit[0].old_values).includes('active') && JSON.stringify(statusAudit[0].new_values).includes('suspended'), 'User status Audit lacks useful change values.');
