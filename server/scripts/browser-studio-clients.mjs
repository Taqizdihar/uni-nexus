...
async function getUser() {
  ...
  try {
    const [rows] = await connection.execute(`SELECT DISTINCT u.id, u.organization_id, u.username FROM users u
      ...
      WHERE u.deleted_at IS NULL AND u.status_code = 'active' AND u.approval_status_code = 'approved' AND p.code = 'studio.clients.write' LIMIT 1`);
    assert(rows.length, 'No active user has studio.clients.write.');
    return rows[0];
...
async function seedFixture(auth) {
  ...
  const client = await call('POST', '/studio/clients', {
    display_name: `PT Uji Browser Klien ${randomBytes(3).toString('hex')}`,
    party_kind: 'company',
    email: 'browser-client@example.com',
    phone: '0811999888',
    city: 'Bandung',
    contacts: [{ full_name: 'Rina Kontak Browser', job_title: 'Marketing', email: 'rina@example.com', is_primary: true }],
  });
  const project = await call('POST', '/studio/projects', {
    client_party_id: client.id,
    project_name: 'Proyek Uji Browser Klien',
    priority_code: 'normal',
    services: [{ description: 'Sesi konsultasi', quantity: 1, unit_price: 2000000 }],
  });
...
async function cleanFixture(fixture) {
  ...
  try {
    ...
    await connection.execute('DELETE FROM party_contacts WHERE party_id = ?', [client.id]);
    await connection.execute("DELETE FROM audit_logs WHERE entity_type = 'party' AND entity_id = ?", [client.id]);
    await connection.execute("DELETE FROM domain_events WHERE entity_type = 'party' AND entity_id = ?", [client.id]);
...
async function run() {
  ...
  try {
    ...
    const profile = await mkdtemp(path.join(os.tmpdir(), 'uni-nexus-studio-clients-browser-'));
    ...
    try {
      ...
      const routes = [
        ['/app/studio/clients', 'Klien', ['Klien Aktif', client.display_name, 'Tambah Klien']],
        ['/app/studio/clients/new', 'Tambah Klien', ['Identitas Klien', 'PIC / Contact Person']],
        [`/app/studio/clients/${client.id}`, client.display_name, ['Ringkasan', 'Kontak', 'Proyek', 'Komersial', 'Aktivitas', 'Klien Studio']],
        [`/app/studio/clients/${client.id}/edit`, 'Edit Klien', ['Identitas Klien']],
        [`/app/studio/projects/new?client=${client.id}`, 'Proyek Baru', [client.display_name]],
      ];

      const outputDir = path.join(root, 'artifacts', `studio-clients-browser-${Date.now()}-${randomBytes(3).toString('hex')}`);
      ...
      cdp.socket.close();
      console.log(`Studio Clients browser acceptance passed. Screenshots: ${outputDir}`);
    } finally {
      child.kill();
      await delay(750);
