...
export class AutomationSensorService {
  async candidates(eventName: string, businessUnitId: number, organizationId?: number, executor: any = pool): Promise<SensorCandidate[]> {
    let resolvedOrganizationId = organizationId;
    const settingsValue = async <T extends SettingValue>(scope: 'craft' | 'studio', group: string, key: string) => {
      if (!resolvedOrganizationId) {
        const [units]: any = await executor.execute('SELECT organization_id FROM business_units WHERE id=? AND is_active=1 LIMIT 1', [businessUnitId]);
        resolvedOrganizationId = units[0] ? Number(units[0].organization_id) : undefined;
      }
      if (!resolvedOrganizationId) throw new Error(`Cannot resolve organization for business unit ${businessUnitId}.`);
      return settingsService.value<T>(resolvedOrganizationId, scope, group, key);
    };
    switch (eventName) {
      case 'order.deadline_approaching': {
        const hours = await settingsValue<number>('craft', 'notifications', 'order_deadline_warning_hours');
        const [rows]: any = await executor.execute(`SELECT id,order_code,status_code,order_type,priority_code,deadline_at,total_amount FROM craft_orders WHERE business_unit_id=? AND deleted_at IS NULL AND status_code NOT IN ('completed','packed','shipped','cancelled','returned') AND deadline_at BETWEEN UTC_TIMESTAMP() AND DATE_ADD(UTC_TIMESTAMP(), INTERVAL ? HOUR)`, [businessUnitId, hours]);
      ...
      case 'studio.project.deadline_approaching': {
        const hours = await settingsValue<number>('studio', 'notifications', 'project_deadline_warning_hours');
        const [rows]: any = await executor.execute(`SELECT id,project_code,project_name,status_code,priority_code,deadline_at,contract_value,project_manager_user_id FROM studio_projects
          WHERE business_unit_id=? AND deleted_at IS NULL AND status_code NOT IN ('completed','paid','cancelled')
            AND deadline_at BETWEEN UTC_TIMESTAMP(3) AND DATE_ADD(UTC_TIMESTAMP(3), INTERVAL ? HOUR)`, [businessUnitId, hours]);
      ...
      case 'studio.project.milestone_due': {
        ...
        const [rows]: any = await executor.execute(`SELECT pm.id milestone_id,pm.title milestone_title,pm.due_at,p.id,p.project_code,p.project_name,p.status_code,p.deadline_at FROM project_milestones pm
          ...
          AND pm.status_code NOT IN ('completed','cancelled') AND pm.due_at BETWEEN UTC_TIMESTAMP(3) AND DATE_ADD(UTC_TIMESTAMP(3), INTERVAL ? HOUR)`, [businessUnitId, hours]);
      ...
      case 'studio.quotation.expiring': {
        const days = await settingsValue<number>('studio', 'notifications', 'quotation_expiry_warning_days');
        const [rows]: any = await executor.execute(`SELECT id,quotation_number,party_id,project_id,status_code,valid_until,total_amount,currency_code FROM quotations
          WHERE business_unit_id=? AND status_code='sent' AND valid_until BETWEEN UTC_DATE() AND DATE_ADD(UTC_DATE(), INTERVAL ? DAY)`, [businessUnitId, days]);
      ...
      case 'studio.invoice.due_soon': {
        const days = await settingsValue<number>('studio', 'notifications', 'invoice_due_soon_days');
        const [rows]: any = await executor.execute(`SELECT id,invoice_number,due_date,balance_due,total_amount,status_code,source_id,currency_code FROM invoices
          WHERE business_unit_id=? AND status_code NOT IN ('draft','paid','void','refunded') AND balance_due>0 AND due_date BETWEEN UTC_DATE() AND DATE_ADD(UTC_DATE(), INTERVAL ? DAY)`, [businessUnitId, days]);
      ...
      case 'studio.invoice.payment_schedule_due': {
        const days = await settingsValue<number>('studio', 'notifications', 'payment_schedule_due_days');
        const [rows]: any = await executor.execute(`SELECT ips.id schedule_id,ips.label,ips.due_date,ips.amount,ips.paid_amount,i.id,i.invoice_number,i.balance_due,i.source_id,i.currency_code FROM invoice_payment_schedules ips
          JOIN invoices i ON i.id=ips.invoice_id WHERE i.business_unit_id=? AND ips.status_code NOT IN ('paid','cancelled') AND ips.amount>ips.paid_amount AND ips.due_date<=DATE_ADD(UTC_DATE(), INTERVAL ? DAY)`, [businessUnitId, days]);
      ...
      case 'studio.asset.maintenance_due': {
        const days = await settingsValue<number>('studio', 'notifications', 'asset_maintenance_warning_days');
        const [rows]: any = await executor.execute(`SELECT a.id,a.asset_code,a.name,a.status_code,m.next_due_at FROM assets a JOIN asset_maintenance_records m ON m.id=(SELECT lm.id FROM asset_maintenance_records lm WHERE lm.asset_id=a.id ORDER BY lm.performed_at DESC,lm.id DESC LIMIT 1)
          WHERE a.business_unit_id=? AND a.deleted_at IS NULL AND m.next_due_at<=DATE_ADD(UTC_TIMESTAMP(3), INTERVAL ? DAY)`, [businessUnitId, days]);
