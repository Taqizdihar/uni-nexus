...
import type { ReportActor } from './reports.types';
...
export class ReportsController {
  ...
  preview = async (req: AuthRequest, res: Response, next: NextFunction) => { try { const access = await reportAccessService.resolve(actor(req), String(req.params.reportCode)); const filters = reportFiltersSchema.parse(req.query); sendSuccess(res, await reportsService.preview(access, filters)); } catch (error) { next(handled(error)); } };
  export = async (req: AuthRequest, res: Response, next: NextFunction) => { try { const request = exportSchema.parse(req.body); const access = await reportAccessService.resolve(actor(req), String(req.params.reportCode), 'export'); const filters = reportFiltersSchema.parse({ ...(request.filters || {}), page: 1, limit: 100 }); sendSuccess(res, await reportsService.export(access, filters, request.format), undefined, 201); } catch (error) { next(handled(error)); } };
  history = async (req: AuthRequest, res: Response, next: NextFunction) => { try { sendSuccess(res, await reportsService.history(actor(req), historySchema.parse(req.query))); } catch (error) { next(handled(error)); } };
