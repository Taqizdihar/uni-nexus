...
import { documentBrandingService } from '../../shared/documents/document-branding.service';
...
export class ReportsService {
  ...
  private async pdf(preview: ReportPreview, organizationId: number) { const branding = await documentBrandingService.resolve(organizationId); return new Promise<Buffer>((resolve, reject) => { const document = new PDFDocument({ margin: 42, size: 'A4', bufferPages: true }); const chunks: Buffer[] = []; document.on('data', chunk => chunks.push(Buffer.from(chunk))); document.on('error', reject); document.on('end', () => resolve(Buffer.concat(chunks))); if (branding.showLogo && branding.logo) { try { document.image(branding.logo, 42, 38, { fit: [54, 42] }); } catch { /* A legacy image must not make an export fail. */ } } const brandX = branding.showLogo && branding.logo ? 106 : 42; document.fontSize(19).fillColor('#242424').text(branding.name, brandX, 42); if (branding.showContact && branding.contact.length) document.fontSize(7).fillColor('#555555').text(branding.contact.join(' • '), brandX, 66, { width: 390, lineBreak: false }); document.y = 94; document.fontSize(15).fillColor('#242424').text(preview.report.name); document.fontSize(9).fillColor('#555555').text(`${preview.report.group} • ${preview.period.start_date} — ${preview.period.end_date} • ${new Date(preview.generated_at).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`); document.moveDown(); document.fillColor('#242424').fontSize(12).text('Ringkasan'); preview.kpis.slice(0, 10).forEach(item => document.fontSize(9).text(`${item.label}: ${item.value ?? '—'}`)); document.moveDown(); document.fontSize(12).text('Data utama'); if (!preview.table.rows.length) document.fontSize(9).text('Belum ada data pada periode ini.'); preview.table.rows.slice(0, 80).forEach((row, index) => { const text = preview.table.columns.slice(0, 5).map(column => `${column.label}: ${safeText(row[column.key])}`).join('  |  '); if (document.y > 740) { document.addPage(); document.fontSize(10).text(`${preview.report.name} (lanjutan)`); } document.fontSize(8).text(`${index + 1}. ${text}`, { width: 510 }); }); if (preview.table.rows.length > 80) document.moveDown().fontSize(8).fillColor('#555555').text(`PDF menampilkan 80 dari ${preview.table.rows.length} baris. Gunakan XLSX atau CSV untuk data tabular lengkap.`); const pages = (document as any).bufferedPageRange().count; for (let index = 0; index < pages; index += 1) { document.switchToPage(index); document.fontSize(7).fillColor('#555555').text(branding.footerText || `${branding.name} • ${preview.report.name}`, 42, document.page.height - 32, { width: 510, align: 'center' }); } document.end(); }); }

  private async binary(format: ExportFormat, preview: ReportPreview, organizationId: number) { if (preview.table.rows.length > EXPORT_CAP) throw new AppError(413, 'REPORT_EXPORT_LIMIT_EXCEEDED', 'Terlalu banyak data untuk diekspor. Persempit filter laporan.'); if (format === 'csv') return { body: this.csv(preview), mimeType: 'text/csv; charset=utf-8' }; if (format === 'xlsx') return { body: await this.xlsx(preview), mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }; return { body: await this.pdf(preview, organizationId), mimeType: 'application/pdf' }; }
  ...
  async export(access: ReportAccess, filters: ReportFilters, format: ExportFormat) {
    ...
    const preview = await this.preview(access, filters); const generated = await this.binary(format, preview, access.actor.organization_id); const filename = `UNI-NEXUS_${safeFile(preview.report.name)}_${preview.period.start_date}_${preview.period.end_date}.${format}`; const saved = await storageService.writeBuffer('report_export', generated.body, filename); const connection = await pool.getConnection();
