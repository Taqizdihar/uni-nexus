...
export class CraftAutomationsRepository {
  async listRules(businessUnitId: number, filters: Record<string, unknown>) {
  ...
  async getRule(id: number, businessUnitId: number, lock = false) {
    const connection = lock ? await pool.getConnection() : null;
    try { const db = connection || pool; const [rows]: any = await db.execute(`SELECT * FROM automation_rules WHERE id=? AND business_unit_id=? ${lock ? 'FOR UPDATE' : ''}`, [id, businessUnitId]); return rows.length ? normalizeRule(rows[0]) : null; }
    finally { connection?.release(); }
  }

  async createRule(input: AutomationRuleInput, context: { organizationId: number; businessUnitId: number; userId: number; moduleCode: string; nextRunAt?: Date | null }) {
    const connection = await pool.getConnection(); await connection.beginTransaction();
    try {
      const [inserted]: any = await connection.execute(`INSERT INTO automation_rules (organization_id,business_unit_id,rule_code,name,description,module_code,trigger_type,trigger_event,trigger_config_json,schedule_timezone,condition_json,action_json,status_code,priority,cooldown_seconds,max_retries,next_run_at,created_by,updated_by) VALUES (?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [context.organizationId, context.businessUnitId, 'TMP', input.name, input.description || null, context.moduleCode, input.trigger_type, input.trigger_event, input.trigger_config_json ? JSON.stringify(input.trigger_config_json) : null, input.schedule_timezone || null, input.condition_json ? JSON.stringify(input.condition_json) : null, JSON.stringify(input.action_json), input.status_code || 'draft', input.priority ?? 100, input.cooldown_seconds ?? 0, input.max_retries ?? 0, context.nextRunAt || null, context.userId, context.userId]);
      const id = Number(inserted.insertId); const code = `AUT-${String(id).padStart(6, '0')}`;
      await connection.execute('UPDATE automation_rules SET rule_code=? WHERE id=?', [code, id]);
  ...
  async updateRule(id: number, next: AutomationRuleInput, context: { organizationId: number; businessUnitId: number; userId: number; moduleCode: string; nextRunAt?: Date | null }) {
    const connection = await pool.getConnection(); await connection.beginTransaction();
    try {
      const [rows]: any = await connection.execute('SELECT * FROM automation_rules WHERE id=? AND business_unit_id=? FOR UPDATE', [id, context.businessUnitId]);
      if (!rows.length) return null; const previous = normalizeRule(rows[0]);
      await connection.execute(`UPDATE automation_rules SET name=?,description=?,module_code=?,trigger_type=?,trigger_event=?,trigger_config_json=?,schedule_timezone=?,condition_json=?,action_json=?,status_code=?,priority=?,cooldown_seconds=?,max_retries=?,next_run_at=?,version_no=version_no+1,updated_by=? WHERE id=?`, [next.name, next.description || null, context.moduleCode, next.trigger_type, next.trigger_event, next.trigger_config_json ? JSON.stringify(next.trigger_config_json) : null, next.schedule_timezone || null, next.condition_json ? JSON.stringify(next.condition_json) : null, JSON.stringify(next.action_json), next.status_code || previous.status_code, next.priority ?? 100, next.cooldown_seconds ?? 0, next.max_retries ?? 0, context.nextRunAt || null, context.userId, id]);
  ...
  async changeStatus(id: number, status: string, context: { organizationId: number; businessUnitId: number; userId: number; nextRunAt?: Date | null }) {
    const connection = await pool.getConnection(); await connection.beginTransaction();
    try { const [rows]: any = await connection.execute('SELECT rule_code,status_code FROM automation_rules WHERE id=? AND business_unit_id=? FOR UPDATE', [id, context.businessUnitId]); if (!rows.length) return null; await connection.execute('UPDATE automation_rules SET status_code=?,next_run_at=?,updated_by=? WHERE id=?', [status, context.nextRunAt || null, context.userId, id]); await this.audit(connection, context, `automation.rule_${status}`, 'automation_rule', id, rows[0].rule_code, `Status aturan ${rows[0].rule_code} menjadi ${status}.`, { status_code: rows[0].status_code }, { status_code: status }); await connection.commit(); return { id, status_code: status }; }
    catch (error) { await connection.rollback(); throw error; } finally { connection.release(); }
  }

  async duplicateRule(id: number, context: { organizationId: number; businessUnitId: number; userId: number }) {
    const rule = await this.getRule(id, context.businessUnitId); if (!rule) return null;
  ...
  async listRuns(businessUnitId: number, filters: Record<string, unknown>) {
    const where = ['r.business_unit_id=?']; const params: unknown[] = [businessUnitId];
