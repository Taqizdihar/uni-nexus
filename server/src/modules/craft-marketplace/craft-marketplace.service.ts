...
export class CraftMarketplaceService {
  ...
  private async cleanupImports() { for (const [token, record] of importPreviews) if (record.expiresAt <= Date.now()) importPreviews.delete(token); }
  ...
  private async parseFile(file: Express.Multer.File): Promise<{ headers: string[]; rows: Array<Record<string, unknown>> }> {
    ...
    const body=await storageService.consumeStagedUpload('marketplace_import',file);
    if(extension==='.csv') { const records=parseCsv(body,{columns:true,skip_empty_lines:true,bom:true,relax_column_count:true,trim:true}) as Array<Record<string,unknown>>; headers=records.length?Object.keys(records[0]):[]; rows=records; }
