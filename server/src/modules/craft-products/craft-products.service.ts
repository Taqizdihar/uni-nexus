...
export class CraftProductsService {
  ...
  async saveProductImage(id: number, file: Express.Multer.File, actor: ProductActor) {
    ...
    const key = storageService.buildKey('products', path.extname(file.originalname), id);
    const stored = await storageService.saveUploadedFile({
      ...
      policy: getStoragePolicy('product_image'), key,
    });
    ...
    try {
      await this.transaction(async connection => {
        ...
        await this.repository.setProductImage(id, stored.key, connection);
        await this.audit(actor, 'product.image_upload', 'product', id, product.sku, `Mengunggah gambar utama produk ${product.name}.`, { image_path: product.image_path }, { image_path: stored.key }, connection);
      });
    } catch (error) {
  ...
  async removeProductImage(id: number, actor: ProductActor) {
    ...
    if (previousPath) await storageService.deleteQuietly(previousPath);
  ...
  async getProductImage(id: number, businessUnitId: number) {
    ...
    return { filePath: storageService.absolutePath(product.image_path), filename: path.basename(product.image_path) };
  }
  ...
  async uploadDesignFile(data: DesignInput, file: Express.Multer.File, actor: ProductActor) {
    ...
    const key = storageService.buildKey('designs', path.extname(file.originalname), destinationFolder);
    const stored = await storageService.saveUploadedFile({
      tempFilePath: file.path, originalName: file.originalname, mimeType: file.mimetype,
      policy: getStoragePolicy('product_design'), key, computeChecksum: true,
    });
    ...
    try {
      const result = await this.transaction(async connection => {
        ...
        const id = await this.repository.createDesignFile({
          ...
          storagePath: stored.key, versionLabel: cleanText(data.version_label), size: stored.sizeBytes,
          checksum: stored.checksumSha256!, isFinal: data.is_final ?? false, uploadedBy: actor.id, notes: cleanText(data.notes),
        }, connection);
        ...
        await this.audit(actor, 'design.upload', 'design_file', id, null, `Mengunggah file desain ${data.name.trim()}.`, null, { ...data, file_name: file.originalname, file_size_bytes: stored.sizeBytes }, connection);
        return { id };
      });
      return result;
    } catch (error) {
  ...
  async downloadDesignFile(id: number, businessUnitId: number) {
    ...
    return { filePath: storageService.absolutePath(design.storage_path), filename: sanitizeOriginalName(design.file_name) };
  }
  ...
  async deleteDesignFile(id: number, actor: ProductActor) {
    ...
    if (storagePath) await storageService.deleteQuietly(storagePath);
