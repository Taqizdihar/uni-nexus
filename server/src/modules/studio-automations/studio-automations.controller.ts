...
export class StudioAutomationsController {
  ...
  private context = async () => { const studio = await getStudioBusinessUnit(); return { organizationId: studio.organizationId, businessUnitId: studio.id }; };
  ...
  test = async (req: Request, res: Response, next: NextFunction) => { try { const context = await this.context(); sendSuccess(res, await this.service.rules.test(id(String(req.params.id)), manualRunSchema.parse(req.body), context.businessUnitId)); } catch (error) { next(zodError(error)); } };
  run = async (req: Request, res: Response, next: NextFunction) => { try { const context = await this.context(); sendSuccess(res, await this.service.queueManualRun(id(String(req.params.id)), manualRunSchema.parse(req.body), actor(req), context), undefined, 202); } catch (error) { next(zodError(error)); } };
