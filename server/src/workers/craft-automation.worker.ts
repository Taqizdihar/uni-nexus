...
import { normalizeRule } from '../modules/craft-automations/craft-automations.repository';
import { AutomationRunService } from '../modules/craft-automations/automation-run.service';
...
export class CraftAutomationWorker {
  readonly id = `automation-${process.pid}-${randomUUID().slice(0, 8)}`;
  readonly runs = new AutomationRunService();

  async recoverStaleLocks() {
    await pool.execute(`UPDATE domain_events SET status_code=IF(attempt_count>=5,'failed','pending'),locked_at=NULL,locked_by=NULL,attempt_count=attempt_count+1,last_error=IF(attempt_count>=5,'Event lock became stale too many times.',last_error) WHERE status_code='processing' AND locked_at<DATE_SUB(UTC_TIMESTAMP(3),INTERVAL 5 MINUTE)`);
  }
  ...
  async processRuns(batchSize = 10) {
    const ids = await this.runs.claim(limit(batchSize), this.id);
    for (const id of ids) await this.runs.execute(id);
    return ids.length;
  }
...
if (isDirectExecution) {
  const worker = new CraftAutomationWorker(); let stopping = false;
  const stop = () => { stopping = true; };
  process.on('SIGINT', stop); process.on('SIGTERM', stop);
  (async () => {
    console.log(`[craft-automation] worker ${worker.id} started`);
    while (!stopping) {
      try { await worker.processOnce(); } catch (error) { console.error('[craft-automation] cycle failed:', sanitizeAutomationError(error)); }
