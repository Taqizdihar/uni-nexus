...
dashboardRouter.get('/', authorize('read'), async (req, res) => {
  ...
  const [openRequests, pendingQuotations, activeOrders, queuedProduction, activePrints, printers, recentFailures, materialUsage, costs, recentActivity] = await Promise.all([
  ...
  res.json({ data: { metrics: { openRequests, pendingQuotations, activeOrders, queuedProduction, activePrints, materialUsageGram: materialUsage._sum.weight_gram?.toString() ?? '0',
