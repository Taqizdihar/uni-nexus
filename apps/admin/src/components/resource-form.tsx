...
import { PrinterPhoto, PrinterRelationSelector } from './special-resource-grids';
...
export function ResourceForm({ resource, record }: { resource: ResourceDefinition; record?: Row }) {
  ...
  const fields = resource.fields.filter((field) => !field.readOnly && (resource.key !== 'printers' || printerOperationalFields.has(field.name)));
  ...
  const printerPhotoSection = resource.key === 'printers' && isCto && record && !record.printer_catalog_id ? <section className="panel form-section"><div className="form-section-heading"><Upload size={18} /><h2>Foto Printer</h2></div><label className="upload-drop"><Upload size={25} /><strong>{printerPhoto ? printerPhoto.name : hasPrinterPhoto ? 'Ganti foto printer' : 'Pilih foto printer'}</strong><span>PNG, JPG, WEBP, atau GIF. Foto unit lama hanya dapat dikelola oleh CTO.</span><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" aria-label="Pilih foto printer" onChange={(event) => setPrinterPhoto(event.target.files?.[0] || null)} /></label>{hasPrinterPhoto && <button type="button" className="button danger small" onClick={() => removePrinterPhoto.mutate()} disabled={removePrinterPhoto.isPending}>Hapus Foto</button>}</section> : resource.key === 'printers' && record ? <section className="panel form-section"><div className="form-section-heading"><span>01</span><h2>Data Printer</h2></div><PrinterPhoto printer={record} className="printer-edit-photo" /><p className="helper-note">Foto dan spesifikasi model dikelola melalui Data Printer oleh CTO.</p></section> : null;
